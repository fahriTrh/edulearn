<div>
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Nilai</h1>
        <p class="text-gray-600">Kelola semua nilai mahasiswa</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-1 mb-6">
        <div class="flex gap-2">
            <button 
                wire:click="switchTab('review')"
                class="flex items-center justify-center gap-2 flex-1 px-6 py-3 rounded-lg font-semibold transition-all {{ $activeTab === 'review' ? 'bg-gradient-to-r from-purple-600 to-purple-800 text-white' : 'text-gray-600 hover:bg-gray-100' }}"
            >
                <x-heroicon-s-document-text class="w-5 h-5" />
                Review & Grading
            </button>
            <button 
                wire:click="switchTab('daftar')"
                class="flex items-center justify-center gap-2 flex-1 px-6 py-3 rounded-lg font-semibold transition-all {{ $activeTab === 'daftar' ? 'bg-gradient-to-r from-purple-600 to-purple-800 text-white' : 'text-gray-600 hover:bg-gray-100' }}"
            >
                <x-heroicon-s-chart-bar class="w-5 h-5" />
                Daftar Nilai
            </button>
            <button 
                wire:click="switchTab('final')"
                class="flex items-center justify-center gap-2 flex-1 px-6 py-3 rounded-lg font-semibold transition-all {{ $activeTab === 'final' ? 'bg-gradient-to-r from-purple-600 to-purple-800 text-white' : 'text-gray-600 hover:bg-gray-100' }}"
            >
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button wire:click="clearSelectionReview" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
                        ← Kembali
                    </button>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="space-y-3 max-h-[600px] overflow-y-auto">
                    ...
                    <button 
                        wire:click="selectClassDaftar({{ $class->id }})"
        ...
        <!-- Grades Content -->
        <div class="lg:col-span-2">
            @if($selectedClassId && $selectedClassDaftar)
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-xl font-semibold text-gray-800">{{ $selectedClassDaftar->title }}</h2>
                        <p class="text-sm text-gray-600">{{ $selectedClassDaftar->code }}</p>
                    </div>
                    <button wire:click="clearSelectionDaftar" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
                        ← Kembali
                    </button>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <button 
                                wire:click="selectAssignmentDaftar({{ $gradeData['assignment_id'] }})"
                ...
                @elseif($viewMode === 'assignment' && $selectedAssignmentDaftar)
                <!-- Assignment View: All Submissions -->
                <div>
                    <div class="mb-6 p-4 bg-purple-50 rounded-lg">
                        <h3 class="font-semibold text-gray-900 mb-1">{{ $selectedAssignmentDaftar->title }}</h3>
                        <p class="text-sm text-gray-600">{{ $selectedAssignmentDaftar->class->title ?? 'N/A' }}</p>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm p-6">
                ...
                <div class="space-y-3 max-h-[600px] overflow-y-auto">
                    @forelse($classes as $class)
                    <button 
                        wire:click="selectClassFinal({{ $class->id }})"
                        class="w-full text-left p-4 border-2 rounded-lg transition-all {{ $selectedClassIdFinal == $class->id ? 'border-purple-600 bg-purple-50' : 'border-gray-200 hover:border-purple-300 hover:bg-gray-50' }}"
                    >
                        <h3 class="font-semibold text-gray-900 mb-1">{{ $class->title }}</h3>
                        <p class="text-sm text-gray-600 mb-2">{{ $class->code }}</p>
                        <div class="flex gap-3 text-xs">
        ...
        <div class="lg:col-span-2">
            @if($selectedClassIdFinal && $selectedClassFinal)
            <div class="bg-white rounded-xl shadow-sm p-6">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-xl font-semibold text-gray-800">{{ $selectedClassFinal->title }}</h2>
                        <p class="text-sm text-gray-600">{{ $selectedClassFinal->code }}</p>
                    </div>
                    <button wire:click="clearSelectionFinal" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition-colors">
                        ← Kembali
                    </button>
                </div>
                ...
                <div class="flex gap-3 mb-6">
                    <button 
                        wire:click="calculateFinalGrades({{ $selectedClassIdFinal }})"
                    ...
                    @if($classSummary && $classSummary['calculated_count'] > 0)
                        @if($classSummary['is_published'])
                            <button 
                                wire:click="unpublishGrades({{ $selectedClassIdFinal }})"
                        ...
                        @else
                            <button 
                                wire:click="publishGrades({{ $selectedClassIdFinal }})"
